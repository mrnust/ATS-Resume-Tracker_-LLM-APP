...
st.title("ATS Resume Tracker:sunglasses:")
...
with col3:
    submit_button2 = st.button("Feedback")
...
if submit_button:   
    ...
    if uploaded_file is not None:
        ...
        st.markdown('<h3 style="color: yellow;text-align: left;">Resume Feedback</h3>', unsafe_allow_html=True)
        
        # Extracting relevant information from JSON response
        match_percentage = response_text.split('"Job Description Match":"')[1].split('"')[0]
        ...
        # Constructing the feedback paragraph
        feedback_paragraph = f"The job description match percentage is {match_percentage}. The missing keywords are {missing_keywords}. Candidate summary: {candidate_summary}. Experience: {experience}"
